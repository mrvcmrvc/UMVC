...
public static class ScrollRectExtensions
{
    ...
    private static void SetTweener(ScrollRect targetScrollRect, float deltaAmount, Action<float> callback)
    {
        ...
        MMVirtualValueTweenerInfo tweenerInfo = new MMVirtualValueTweenerInfo
        {
            Callback = onTweenerUpdate,
            Duration = 0.5f,
            Ease = MMTweeningEaseEnum.OutCubic,
            From = targetScrollRect.content.position.y,
            To = targetScrollRect.content.position.y + deltaAmount,
            IgnoreTimeScale = true
        };

        MMVirtualValueTweener newTweener = new MMVirtualValueTweener(tweenerInfo);

        MMVirtualValueTweenController.Instance.StartTweener(newTweener);
    }
}
